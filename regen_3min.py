#!/usr/bin/env python3
"""Generate 3min datasets mirroring all existing 5min dataset parameters."""
import json, os, subprocess, sys, time

MANIFEST = os.path.join(os.path.dirname(os.path.abspath(__file__)), "logic", "heatmap_data", "manifest.json")

def period_to_env(p):
    if p.endswith("y"): return p[:-1] + " years"
    return p

with open(MANIFEST) as f:
    datasets = [d for d in json.load(f)["datasets"] if d["timeframe_label"] == "5min"]

total = len(datasets)
print(f"=== Generating {total} 3min datasets (mirroring 5min params) ===\n")

failed = []
for i, d in enumerate(datasets, 1):
    ds_id = d["id"].replace("_5min_", "_3min_")
    env = {
        **os.environ,
        "FVG_TICKER":          "NQ",
        "FVG_TIMEFRAME":       "3min",
    ...
    print(f"\n[{i}/{total}] {ds_id}")
    print(f"  period={d['data_period']} session={d['session_period_minutes']}min "
